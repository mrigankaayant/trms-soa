package com.ayantsoft.trms.resourcing.info;

public class DatabaseInfo {
	
	public static final String DATABASENAME = "trms_resourcing";
	public static final String IP = "localhost";
	public static final int PORT = 27017;
	
	public static final String CANDIDATE_COLLECTION = "candidate";
	public static final String IMMIGRATION_COLLECTION = "immigration";
	public static final String COURSE_COLLECTION = "course";
	public static final String PREF_LOCATION_COLLECTION = "locations";
	public static final String RECRUITMENT_SOURCE_COLLECTION = "recruitment_source";
	public static final String PAYTYPE_COLLECTION = "pay_type";
	public static final String DOUMENTS_COLLECTION = "documents";
	public static final String CANDIDATE_STATUS_COLLECTION = "candidate_status";
	public static final String FOLLOWUP_COLLECTION = "follow_up";
	public static final String MAX_FOLLOWUP_COLLECTION = "max_followup";
	public static final String PHONE_LOGS_COLLECTION = "phone_logs";
	

}
